    ...
    <div class="w-full max-w-[420px]">
      ...
      <form @submit.prevent="onSubmit" class="mt-10">
        <fieldset :disabled="form.isSubmitting.value" class="grid gap-5">
          ...
          <UiDivider label="or" />
          <UiButton @click="signInWithGoogle" type="button" class="w-full flex items-center gap-2" variant="outline">
            <Icon name="logos:google-icon" /> Sign in with Google
          </UiButton>
          <UiButton @click="signInWithGithub" type="button" class="w-full flex items-center gap-2" variant="outline">
      ...
      <p class="mt-10 text-center text-sm">
        No account yet?
        <NuxtLink class="text-sm font-semibold text-primary hover:underline" to="/auth/signup">Sign up here</NuxtLink>
      </p>
    </div>
  </UiContainer>
</template>
